...
from comp_reasoning.models import MLPModel, EBMDiffModel, EBMCrossWorld
...
from comp_reasoning.data.crossword import CrosswordRowDataset, idxs_to_uppercase_string, load_embeddings_dict, load_instances, instance_to_torch
...
def experiment():
    ...
    emb_dict = load_embeddings_dict(params.embedding_file)
    instances = load_instances(params.instances_file, split='test')

    for i, instance in enumerate(instances):

        _, hw, _, vw = instance
        instance = instance_to_torch(instance, emb_dict)
        h_words, h_embeddings, v_words, v_embeddings = instance

        h_words = torch.cat(h_words, dim=0).reshape(-1)[None, :]
        v_words = torch.cat(v_words, dim=0).reshape(-1)[None, :]
        
        h_embeddings = [emb.to(device) for emb in h_embeddings]
        v_embeddings = [emb.to(device) for emb in v_embeddings]

        h_embeddings = torch.stack(h_embeddings, dim=0)
        v_embeddings = torch.stack(v_embeddings, dim=0)

        embeddings = {
            'h_words': h_embeddings,
            'v_words': v_embeddings,
        }
        
        batch_size = h_words.shape[0]
        ...
        if sampler_type in ('reverse_diffusion', 'ULA', 'MALA', 'UHMC', 'HMC'):
            ...
            x_sampled = torch.rand(h_words.shape).to(device)
        else:
            raise ValueError(f"Invalid sampler type: {sampler_type}")
        
        x_sampled = x_sampled.reshape(batch_size, 5 * 5, params.vocab_size)
        pred_sample = torch.argmax(x_sampled, dim=-1)
        target_sample = h_words.reshape(batch_size, 5 * 5, params.vocab_size)
        ...
        for ii in range(batch_size):    
            ...
            matching_letters = (target_sample[ii] == pred_sample[ii]).sum().item()

            # Count matching words
            matching_words = 0

            for wi in range(5):
                target_word = idxs_to_uppercase_string(target_sample[ii, wi * 5:(wi + 1) * 5].cpu().numpy())
                pred_word = idxs_to_uppercase_string(pred_sample[ii, wi * 5:(wi + 1) * 5].cpu().numpy())
                is_match = target_word == pred_word
                if is_match:
                    matching_words += 1
                print(f"H {wi}: Sampled: {pred_word}, Target: {target_word}, Match: {is_match}")

            # Vertical words (columns)
            for wi in range(5):
                target_word = idxs_to_uppercase_string(target_sample[ii, wi::5].cpu().numpy())
                pred_word = idxs_to_uppercase_string(pred_sample[ii, wi::5].cpu().numpy())
                is_match = target_word == pred_word
                if is_match:
                    matching_words += 1
                print(f"V {wi}: Sampled: {pred_word}, Target: {target_word}, Match: {is_match}")

            print(f"Matching letters: {matching_letters} / 25")
            print(f"Matching words: {matching_words} / 10")
